...
const before = function (options = {}) { // eslint-disable-line no-unused-vars
  return function (hook) {
    ...
    return new Promise((resolve, reject) => {
      ...
      } else if ('_classifiedContent' in hook.params.query) {
        delete hook.params.query._classifiedContent;
        hook.params.query['$or'] = [
          { contentReviewers: { $exists: true } }
        ];
        resolve(hook);
        // hook.pamams.query
      } else if ('_classifiedTitle' in hook.params.query) {
        delete hook.params.query._classifiedTitle;
        hook.params.query['$or'] = [
...
const after = function (options = {}) { // eslint-disable-line no-unused-vars
  return function (hook) {
    ...
    // that resolves with the `hook` object for asynchronous operations
    if (!('_totals' in hook.params.query
      || '_classifiedContent' in hook.params.query
      || '_classifiedTitle' in hook.params.query
      || '_classified' in hook.params.query)
      && hook.result.data[0].classifiers) {
